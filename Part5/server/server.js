...
stats = (action, name, time) => {
    fs.readFile('./server/json/stats.json', 'utf-8', (err, data) => {
        if (err) {
            ...
            fs.writeFile('./server/json/stats.json', JSON.stringify(stats), (err) => {
                if (err) {
                    console.log(`Статистика не ведёться. Ошибка: ${err}`);
                } else {
...
app.post('/addToCart', (req, res) => {
    fs.readFile('./server/json/cart.json', 'utf-8', (err, data) => {
        if (err) {
            res.send('{"result": 0}');
        } else {
            ...
            fs.readFile('./server/json/total.json', 'utf-8', (err, data) => {
                if (err) {
                    ...
                    fs.writeFile('./server/json/total.json', JSON.stringify(currentTotal), (err) => {
                        if (err) {
                            console.log(`Цена не записана. Ошибка: ${err}`)
                        } else {
            ...
            fs.writeFile('./server/json/cart.json', JSON.stringify(cart), (err) => {
                if (err) {
                    res.send('{"result": 0}');
                } else {
